...
for /F %%i in ('docker ps --all --quiet --filter "ancestor=dragon-framework-gradle-webflux:1.0.0-SNAPSHOT" --filter "status=running"') do (
...
for /F %%i in ('docker ps --all --quiet --filter "ancestor=dragon-framework-gradle-webflux:1.0.0-SNAPSHOT" --filter "status=exited"') do (
...
for /F %%i in ('docker images --all --quiet dragon-framework-gradle-webflux:1.0.0-SNAPSHOT') do (
...
echo [32m构建镜像开始[0m
docker build --no-cache=true --tag dragon-framework-gradle-webflux:1.0.0-SNAPSHOT --file dockerfile .
echo [32m构建镜像完成[0m
echo.
